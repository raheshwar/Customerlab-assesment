...
const LabForm = () => {
  ...
  const add = () => {
    if (!selected && selectedSchema.length == 0) {
      return messageApi.open({
        type: 'error',
        content: 'Please select schema',
      });
    }
    if (!selected && selectedSchema.length > 0) {
      return messageApi.open({
        type: 'error',
        content: 'Please select another schema',
      });
    }
    else if (selectedSchema.includes(selected)) {
      messageApi.open({
        type: 'error',
        content: 'Schema already added',
      });
  ...
  const savesegment = async () => {
    ...
    if (!segmentName) {
      ...
      return messageApi.open({
        type: 'error',
        content: 'Please Add atleast one schema',
      });
    }
    else {
      let payloadSchma = []
      options.forEach(element => {
        if (selectedSchema.includes(element.Value)) {
          ...
          let modifiedLabel = words.join(' ');
          let obj = { [element.Value]: modifiedLabel };
          payloadSchma.push(obj);
        }
      });
      let payload =
      {
      ...
      if (res?.status === 200) {
        ...
        messageApi.open({
          type: 'error',
          content: 'Due to the wrong url you couldnot able to save schema',
        });
      }
    }
  }
  return (
    ...
    <div className=" ">
      ...
      <div onClick={showDrawer} className="border-2 text-gray-500 font-medium hover:bg-gray-300 hover:text-white border-gray-300 ml-28 text-center w-32 py-2 mt-10 ">
        <button type="primary" >
          Save Segment
        </button>
      </div>
      ...
      >
        <div className="">
          <h2 className="text-sm font-medium">Enter the Name of the Segment</h2>
          <input
            type="text"
        ...
        <div className="gap-3  flex items-center justify-start mt-20">
          ...
          <div className="border-2 border-red-500 px-4 py-2 font-medium rounded-md text-red-500" onClick={onClose}><button>Cancel</button></div>
